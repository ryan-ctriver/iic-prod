(() => {
  ...
  var { setSites } = useDataStore.getState();
  async function initializeData() {
    const sites = document.querySelectorAll(".data_site");
    const data = await mapSites(sites);
    setSites(data);
  ...
  // src/modules/process-sites-count.js
  var { getSites, getLang } = useDataStore.getState();
  function processSitesCount() {
    const sites = getSites();
    const lang = getLang();
    const sitesCountElement = document.querySelector("#sites-count");
    if (sitesCountElement && sites.length > 0) {
      sitesCountElement.display = "block";
      sitesCountElement.textContent = lang === "en" ? `${sites.length} monitoring site${sites.length > 1 ? "s" : ""}` : `${sites.length} sitio${sites.length > 1 ? "s" : ""} de monitoreo`;
      sitesCountElement.style.display = "block";
    } else {
      sitesCountElement.remove();
    }
  }
  ...
  async function initialize() {
    const mapSection2 = document.querySelector(".section-fade-in-animation");
    const data = await initializeData();
    processSitesCount();
    if (data.length > 0) setInitialized();
    else mapSection2.remove();
  }
  var isMobile = window.innerWidth <= 767;
  var header = document.querySelector("#navbar");
  var mapSection = document.querySelector(".section.padding-3.section-fade-in-animation");
  var mapFirstWrapper = document.querySelector(".grid._12-col.radius-2");
  if (isMobile) {
    mapSection.style.padding = "0px";
    mapFirstWrapper.style["border-radius"] = "0px";
    header.remove();
  }
  function setInitialized() {
    const mapSection2 = document.querySelector(".section-fade-in-animation");
    mapSection2.style.opacity = "1";
