(() => {
  ...
  async function initializeData() {
    const statesOptions = document.querySelectorAll(".map_filter-option--states");
    const countiesOptions = document.querySelectorAll(".map_filter-option--counties");
    const townsOptions = document.querySelectorAll(".map_filter-option--towns");
    const sitesOptions = document.querySelectorAll(".map_filter-option--sites");
    const watershedOptions = document.querySelectorAll(".map_filter-option--watersheds");
    const waterbodyOptions = document.querySelectorAll(".map_filter-option--waterbodies");
    const organizationsOptions = document.querySelectorAll(".map_filter-option--organizations");
    const sites = await mapSites(sitesOptions);
    if (!sites) return false;
    setSites(sites);
    const states = mapStates({ statesOptions, countiesOptions, townsOptions });
    const counties = mapCounties(countiesOptions, townsOptions);
    const towns = mapTowns(townsOptions, sitesOptions);
    const watersheds = mapWaterSheds(watershedOptions, waterbodyOptions, sitesOptions);
    const waterbodies = mapWaterBodies(waterbodyOptions, sitesOptions);
    const organizations = mapOrganizations(organizationsOptions, sitesOptions);
    if (!sites || !states || !counties || !towns || !watersheds || !waterbodies || !organizations)
      return false;
    setStates(states);
    setCounties(counties);
    setTowns(towns);
    setWaterSheds(watersheds);
  ...
  async function mapSites(sitesOptions) {
    console.log("Sites options:");
    console.log(sitesOptions.length);
    ...
    sitesOptions.forEach((item) => {
      const siteOrganizations = item.querySelectorAll(".nested_organizations");
      const image = item.querySelector(".site-image");
      const siteDate = item.getAttribute("data-date");
      data.push({
        id: item.value,
        name: item.textContent,
        latitude: item.getAttribute("data-latitude"),
        longitude: item.getAttribute("data-longtitude"),
        image: !image?.classList?.contains("w-dyn-bind-empty") ? image?.src : void 0,
        description: item.getAttribute("data-description"),
        waterbody: item.getAttribute("data-waterbody"),
        watershed: item.getAttribute("data-watershed"),
        quality: +item.getAttribute("data-quality"),
        state: item.getAttribute("data-state"),
        county: item.getAttribute("data-county"),
        town: item.getAttribute("data-town"),
        date: siteDate ? new Date(siteDate) : void 0,
        organizations: Array.from(siteOrganizations).map(
          (item2) => item2.getAttribute("data-organization")
        ),
        predictionDry: {
    ...
    const filteredSites = data.filter((site) => {
      const isValid = site.organizations.length > 0 && site.quality && site.date;
      return isValid;
    });
    const weatherDataResponse = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        stations: filteredSites.map((station) => station.id)
      })
    });
  ...
  function mapStates({ statesOptions, countiesOptions, townsOptions }) {
    ...
    statesOptions.forEach((item) => {
      data.push({
        id: item.value,
        name: item.textContent,
        sites: sites.filter((site) => site.state === item.value).map((site) => site.id),
        counties: Array.from(countiesOptions).filter((option) => option.getAttribute("data-state") === item.value).map((option) => option.value),
        towns: Array.from(townsOptions).filter((option) => option.getAttribute("data-state") === item.value).map((option) => option.value)
      });
    });
    const filteredData = data.filter(
      (state) => state.counties.length > 0 && state.counties.length > 0 && sites.find((site) => site.state === state.id)
    );
    return filteredData;
  }
  function mapCounties(countiesOptions, townsOptions) {
    const data = [];
    const sites = getSites();
    countiesOptions.forEach((item) => {
      data.push({
        id: item.value,
        name: item.textContent,
        state: item.getAttribute("data-state"),
        sites: sites.filter((site) => site.county === item.value).map((site) => site.id),
        towns: Array.from(townsOptions).filter((option) => option.getAttribute("data-county") === item.value).map((option) => option.value)
      });
    });
    const filteredData = data.filter(
      (county) => county.state && county.sites.length > 0 && county.towns.length > 0 && sites.find((site) => site.county === county.id)
    );
    return filteredData;
  }
  function mapTowns(townsOptions, sitesOptions) {
    const data = [];
    const sites = getSites();
    townsOptions.forEach((item) => {
      data.push({
        id: item.value,
        name: item.textContent,
        state: item.getAttribute("data-state"),
        county: item.getAttribute("data-county"),
        sites: sites.filter((site) => site.town === item.value).map((site) => site.id)
      });
    });
    const filteredData = data.filter(
      (town) => town.state && town.county && town.sites.length > 0 && sites.find((site) => site.town === town.id)
    );
    return filteredData;
  }
  function mapWaterSheds(watershedOptions, waterbodyOptions) {
    const data = [];
    const sites = getSites();
    watershedOptions.forEach((item) => {
      data.push({
        id: item.value,
        name: item.textContent,
        boundary: item.getAttribute("data-boundary-outline"),
        waterbodies: Array.from(waterbodyOptions).filter((option) => option.getAttribute("data-watershed") === item.value).map((option) => option.value),
        sites: sites.filter((site) => site.watershed === item.value).map((site) => site.id)
      });
    });
    const filteredData = data.filter(
      (watershed) => watershed.waterbodies.length > 0 && sites.find((site) => site.watershed === watershed.id)
    );
    return filteredData;
  }
  function mapWaterBodies(waterbodyOptions) {
    const data = [];
    const sites = getSites();
    waterbodyOptions.forEach((item) => {
      data.push({
        id: item.value,
        name: item.textContent,
        watershed: item.getAttribute("data-watershed"),
        boundaryLakes: item.getAttribute("data-boundary-lakes"),
        boundaryRivers: item.getAttribute("data-boundary-rivers"),
        sites: sites.filter((site) => site.waterbody === item.value).map((site) => site.id)
      });
    });
    const filteredData = data.filter(
      (waterbody) => waterbody.sites.some((site) => sites.map((item) => item.id).includes(site)) && waterbody.watershed
    );
    return filteredData;
  }
  function mapOrganizations(organizationsOptions, sitesOptions) {
    const data = [];
    organizationsOptions.forEach((item) => {
      const organization = {
        id: item.value,
        name: item.textContent,
        logo: item.getAttribute("data-logo"),
        link: item.getAttribute("data-link"),
        sites: Array.from(sitesOptions).filter((site) => {
          const nestedOrganizations = site.querySelectorAll(".nested_organizations");
          return Array.from(nestedOrganizations).some(
            (nested) => nested.getAttribute("data-organization") === item.value
          );
  ...
  function processLoader() {
    ...
    const timeline2 = gsap3.timeline({ repeat: -1, repeatDelay: 0 });
    timeline2.to(drop, {
      duration: 0.5,
    ...
    async (listInstances) => {
      ...
      if (listInstances.length > 0) {
        listInstances.forEach((list) => {
          if (list.listElement.querySelector(".map_filter-option--sites"))
            list.addHook("render", async () => {
              await initialize();
              console.log("Hooked after render.");
            });
        });
      } else {
        await initialize();
  ...
  function setInitialized() {
    ...
    if (window.buildMapFilters) window.buildMapFilters();
  }
})();
